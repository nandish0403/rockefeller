        ...
        <div style={{ animation: "zdFadeUp 0.35s ease both" }}>
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "flex-end",
              ...
              {(weather.length > 0 ? weather : Array(12).fill(null)).slice(0, 12).map((w, i) => {
                const val   = w?.rainfall_mm ?? Math.random() * 10;
                const max   = Math.max(...(weather.map(x => x.rainfall_mm ?? 0)), 1);
